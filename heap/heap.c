...
struct Heap {
	int data[15]; //fixed length, for edu purposes.
	int length;
	int currIndex;
};
 ...
 *
 *  "Why take the largest of the children" - Because the max heap's key property is
 *  that the parent be larger than it's children. Two child nodes are not related to
 *  each other in any way. That's an implementation detail. Relationships are strictly parent-child.
 */
...
/**
 * Traverse from the leaf node "index" to the top
 */
...
int main()
{
	struct Heap heap = {.length = 15, .currIndex = 0};
